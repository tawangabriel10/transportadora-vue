  ...
  <div>
      <v-col>
        ...
        <v-row>
          <v-col>
            <custom-input
              :header="{ type: 'number', text: 'Nº Ordem de Serviço', value: 'numOrdemServico', rules: [ 'required' ] }"
              :edited-item="form"
            />
          </v-col>
          <v-col>
            <custom-input
              :header="{ type: 'date', text: 'Data de emissão', value: 'dataEmissao', rules: [ 'required'] }"
              :edited-item="form"
            />
          </v-col>
          <v-col>
          ...
          <v-col>
            <custom-input
              :header="{ type: 'date', text: 'Previsão da coleta', value: 'dataColeta', rules: [ 'required' ] }"
              :edited-item="form"
            />
          </v-col>
          <v-col>
        ...
        <v-row>
          <v-col>
            <custom-input
              :header="{ type: 'text', text: 'Local de coleta', value: 'localColeta', rules: [ 'required' ]  }"
              :edited-item="form"
            />
          </v-col>
          <v-col>
          ...
          <v-col>
            <v-select label="Status" :items="todosStatus" v-model="form.status"></v-select>
          </v-col>
        </v-row>
        ...
        <v-row>
          <v-col>
            <custom-input
              :header="{ type: 'radio', text: 'Tipo de solicitante', value: 'tipoSolicitante', items: [{ id: 'PF', nome: 'Física' },{ id: 'PJ', nome: 'Jurídica' }], rules: [ 'required' ] }"
              :edited-item="form"
            />
          </v-col>
          <v-col>
            <custom-input
              :header="{ type: 'text', text: 'CPF/CNPJ', value: 'cpfCnpjSolicitante', mask: (form.tipoSolicitante === 'PJ' ? '##.###.###/####-##' : '###.###.###-##'), rules: [ 'required'] }"
              :edited-item="form"
            />
          </v-col>
        </v-row>
...
export default {
      ...
      customInputs: {
        ...
        destinatarioProps: {
          header: {
            ...
            onSelectRow: async (item) => {
              await this.buildClientePessoaCustomInput(
                item.id,
                "idDestinatario",
  ...
  methods: {
    ...
    buildClientePessoaCustomInput(
      id,
      key = "idRemetente",
      labelInput = "Código do Remetente",
      dynamicAttribute = "remetente",
      customInputName = "remetenteProps"
    ) {
    ...
    buildSolicitanteCustomInput(
      id,
      key = "idSolicitante",
      labelInput = "Nome do Solicitante",
      dynamicAttribute = "solicitante",
      customInputName = "solicitanteProps"
    ) {
      const getItemData = async (item) => {
        this.form[key] = item.id;
        const data = (
